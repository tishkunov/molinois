...
function* sendMessageWorker(action) {
  try {
    ...
    const data = yield call(() => {
      return fetch('http://lk/dashboard/sendMessage.php', {
        method:'POST',
        body: action.payload
      })
      .then(res => res.json())
...
function* buyTokensWorker(action) {
  try {
    const data = yield call(() => {
      return fetch('http://lk/dashboard/buyTokens/index.php', {
        method:'POST',
        body: action.payload
      })
      .then(res => res.json())
    ...
    if (data.error === 'ok') {
      ...
      const data2 = yield call(() => {
        return fetch('http://lk/dashboard/buyTokens/transactioninfo.php', {
          method:'POST',
          body: dataTrans
        })
        .then(res => res.json())
...
function* sendCompleteTransactionWorker(action) {
  try {
    ...
    const data = yield call(() => {
      return fetch('http://lk/dashboard/buyTokens/transactionComplete.php', {
        method:'POST',
        body: action.payload
      })
      .then(res => res.json())
...
function* fetchUserDataWorker(action) {
  try {
    const data = yield call(() => {
        return fetch('http://lk/confirm/getSession.php', {
          method:'POST',
          body:action.payload
        })
              .then(res => res.json())
...
function* getTransactionsDataWorker(action) {
  try {
    const data = yield call(() => {
        return fetch('http://lk/dashboard/transactionsData.php', {
          method:'POST',
          body:action.payload
        })
              .then(res => res.json())
